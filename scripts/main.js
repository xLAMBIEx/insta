    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        if (res === "Contact Added") {
          ...
          alert("Contact Request Failed: one or more fields are empty");
        }
      },
      error: function (err) {
        console.log(err);
        ...
        res.map(function(value, index) {
          $("#designs-container").append(`
            <div class="swiper-slide">
              ...
              <img src="media/designs/` + value.featuredImage + `.jpg" class="img-fluid" alt="Design Featured Image" />
              <h3 class="text-left m-3 text-smaller">
                ` + formatter.format(value.price) + `
                <span class="float-right">
                  <i class="fas fa-bed"></i> ` + value.bedrooms + `
                  <i class="fas fa-bath ml-3"></i> ` + value.bathrooms + `
                  <i class="fas fa-arrows-alt ml-3"></i> ` + value.size + `m²
                </span>
              </h3>
              <button id="` + value.id + `" class="view-details btn btn-block btn-secondary mt-2 select-house">View Details</button>
            </div>
          `);
        })
  ...
  $.ajax({
    ...
    success: function (res) {
      if (res !== "No Results" && res !== "Invalid Parameter") {
        ...
        $('#designDetailsGalleryImage1').attr('src', "media/designs/" + res.image1 + ".jpg");
        $('#designDetailsGalleryImage2').attr('src', "media/designs/" + res.image2 + ".jpg");
        $('#designDetailsGalleryImage3').attr('src', "media/designs/" + res.image3 + ".jpg");
        ...
        $("#designDetailsBuy").attr('href', "payment/checkout.php?did=" + res.id);
        ...
        res.map(function(value, index) {
          $("#active-designs-table").append(`
            <tr>
              ...
              <td>R` + value.price + `</td>
              <td>
                <button id="` + value.id + `" class="edit-design btn btn-block btn-outline-dark p-1">Edit</button>
                <button id="` + value.id + `" class="delete-design btn btn-block btn-outline-dark p-1">Delete</button>
              </td>
            </tr>
          `);
        })
...
function getAdminDesignDetails(id) {
  $.ajax({
    ...
    success: function(res) {
      ...
      $("#uploadFeaturedImageMsg").text(res.featuredImage + ".jpg uploaded").show();
      $("#uploadGalleryImage1Msg").text(res.image1 + ".jpg uploaded").show();
      $("#uploadGalleryImage2Msg").text(res.image2 + ".jpg uploaded").show();
      $("#uploadGalleryImage3Msg").text(res.image3 + ".jpg uploaded").show();
        ...
        res.map(function(value, index) {
          $("#payment-history-table").append(`
            <tr>
              ...
              <td>
                ` + value.buyerName + ` <br>
                ` + value.buyerContact + ` <br>
                ` + value.buyerEmail + `
              </td>
              <td>` + value.item + `</td>
              <td>R` + value.payment + `</td>
        ...
        res.map(function(value, index) {
          $("#active-users-table").append(`
            <tr>
              ...
              <td>` + value.email + `</td>
              <td>
                <button id="` + value.id + `" class="edit-user btn btn-block btn-outline-dark p-1">Edit</button>
                <button id="` + value.id + `" class="delete-user btn btn-block btn-outline-dark p-1">Delete</button>
              </td>
            </tr>
          `);
        })
